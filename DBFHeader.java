...
public class DBFHeader {
	...
	private String languageDriverName; /* 32-63 Only if DB7 */
	private int reserv5; /* 64-67 Only if DB7 */
	DBFField[] fieldArray; /* each 32 bytes */
	DBF7Field[] fieldArray7;/* each 48 bytes */
	DBFField[] userFieldArray;
	DBF7Field[] userFieldArray7;
	...
	/**
	 * Flag indicating if 2-byte (extended) length character fields should be
	 * supported (see DBFField.adjustLengthForLongCharSupport(), default: true).
	 */
...
//			this.reserv5 =  dataInput.readInt();
		...
		if (isDB7()) {
			/* 48 each */
			while ((field = DBFField.createFieldDB7(dataInput, this.usedCharset,
					supportExtendedCharacterFields)) != null) {
				v_fields.add(field);
			}
		} else {
			/* 32 each */
			boolean useFieldFlags = supportsFieldFlags();
			while ((field = DBFField.createField(dataInput, this.usedCharset, useFieldFlags,
					supportExtendedCharacterFields)) != null) {
				v_fields.add(field);
			}
	...
	private boolean supportsFieldFlags() {
		return this.signature == 0x2 || this.signature == 0x30 || this.signature == 0x31 || this.signature == 0xF5
				|| this.signature == 0xFB;
	}
	...
	void writeDB3(DataOutput dataOutput) throws IOException {
		...
		for (DBF7Field field : this.fieldArray7) {
			field.write(dataOutput, getUsedCharset());
		}
		dataOutput.writeByte(this.terminator1); /* n+1 */
	}

	void writeDB7(DataOutput dataOutput) throws IOException {
		dataOutput.writeByte(this.signature); /* 0 */
		...
		dataOutput.writeShort(DBFUtils.littleEndian(this.reserv4)); /* 30-31 */
		dataOutput.write(DBFUtils.textPadding("", usedCharset, 32, DBFAlignment.LEFT, (byte) 0)); /* 32-63 */
		dataOutput.write(DBFUtils.textPadding("", usedCharset, 4, DBFAlignment.LEFT, (byte) 0)); /* 64-67 */
		for (DBF7Field field : this.fieldArray7) {
			field.write(dataOutput, getUsedCharset());
		}
		dataOutput.writeByte(this.terminator1); /* n+1 */
	...
	private short findHeaderLength() {
		if (isDB7())
			return (short) (1 + 3 + 4 + 2 + 2 + 2 + 1 + 1 + 4 + 4 + 4 + 1 + 1 + 2 + 32 + 4
					+ (48 * this.fieldArray7.length) + 1);
		else
			return (short) (1 + 3 + 4 + 2 + 2 + 2 + 1 + 1 + 4 + 4 + 4 + 1 + 1 + 2 + (32 * this.fieldArray.length) + 1);
	}
	...
	/**
	 * Sets whether 2-byte (extended) length character fields should be supported
	 * (see DBFField.adjustLengthForLongCharSupport(), default: true).
	 * 
