...
public class DBFWriter extends DBFBase implements java.io.Closeable {
	...
	/**
	 * Creates a DBFWriter wich write data to the given OutputStream. Uses default
	 * charset iso-8859-1
	 * 
	 ...
	 * @exception DBFException if the passed in file does exist but not a valid DBF
	 *                         file, or if an IO error occurs.
	 */
	...
	/**
	 * Sets fields definition. To keep maximum compatibility a maximum of 255
	...
	public void setFields(DBFField[] fields) {
		...
		for (DBFField field : fields) {
			if (!field.getType().isWriteSupported()) {
				throw new DBFException("Field " + field.getName() + " is of type " + field.getType()
	 ...
	 * @param fields fields definition
	 */
	public void setFields(DBF7Field[] fields) {
		this.header.setDB7();
		if (this.closed) {
			throw new IllegalStateException("You can not set fields to a closed DBFWriter");
		}
		...
		for (DBF7Field field : fields) {
			if (!field.getType().isWriteSupported()) {
				throw new DBFException("Field " + field.getName() + " is of type " + field.getType()
						+ " that is not supported for writting");
			}
		}
		this.header.fieldArray7 = new DBF7Field[fields.length];
		for (int i = 0; i < fields.length; i++) {
			this.header.fieldArray7[i] = new DBF7Field(fields[i]);
		}
	...
	public void addRecord(Object[] values) {
		...
		if (this.header.isDB7()) {
			if (this.header.fieldArray7 == null)
				throw new DBFException("Fields should be set before adding records");

			if (values.length != this.header.fieldArray7.length)
				throw new DBFException("Invalid record. Invalid number of fields in row");

			for (int i = 0; i < this.header.fieldArray7.length; i++) {
				Object value = values[i];
				if (value == null) {
					continue;
				}

				switch (this.header.fieldArray7[i].getType()) {
				...
				default:
					throw new DBFException(
							"Unsupported writting of field type " + i + " " + this.header.fieldArray[i].getType());
				}
		...
		if (this.raf == null) {
			...
			try {
				if (this.header.isDB7())
					writeRecordDB7(this.raf, values);
				else
	...
	private void writeToStream(OutputStream out) {
		try {
			...
			for (Object[] record : this.v_records) {
				if (this.header.isDB7())
					writeRecordDB7(outStream, record);
				else
	...
	private void writeRecord(DataOutput dataOutput, Object[] objectArray) throws IOException {
		...
		for (int j = 0; j < this.header.fieldArray.length; j++) {
			...
			case CHARACTER:
				...
				dataOutput.write(DBFUtils.textPadding(strValue, getCharset(), this.header.fieldArray[j].getLength(),
						DBFAlignment.LEFT, (byte) ' '));
			...
			case FLOATING_POINT:
				...
				if (objectArray[j] != null) {
					...
					dataOutput.write(DBFUtils.textPadding(" ", getCharset(), this.header.fieldArray[j].getLength(),
							DBFAlignment.RIGHT, (byte) ' '));
				}
	...
	private void writeRecordDB7(DataOutput dataOutput, Object[] objectArray) throws IOException {
		dataOutput.write((byte) ' ');
		for (int j = 0; j < this.header.fieldArray7.length; j++) {
			/* iterate throught fields */
			switch (this.header.fieldArray7[j].getType()) {
			...
			case CHARACTER:
				...
				dataOutput.write(DBFUtils.textPadding(strValue, getCharset(), this.header.fieldArray7[j].getLength(),
						DBFAlignment.LEFT, (byte) ' '));
			...
			case FLOATING_POINT:
				...
				if (objectArray[j] != null) {
					dataOutput.write(DBFUtils.doubleFormating((Number) objectArray[j], getCharset(),
							this.header.fieldArray7[j].getLength(), this.header.fieldArray7[j].getDecimalCount()));
				} else {
					dataOutput.write(DBFUtils.textPadding(" ", getCharset(), this.header.fieldArray7[j].getLength(),
							DBFAlignment.RIGHT, (byte) ' '));
				}
			...
			default:
				throw new DBFException("Unknown field type " + this.header.fieldArray7[j].getType());
			}
		}
	}
	...
	/**
	 * Get de underlying RandomAccessFile. It can be null if OutputStream
	 * constructor is used.
	 * 
